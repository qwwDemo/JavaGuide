...
再以CountDownLatch以例，任务分为N个子线程去执行，state也初始化为N（state的初始化不一定为线程数N，可以大于等于线程数N，只需要保证state的初始值能够被多次调用countDown()进行-1操作直到0就可以）。这N个子线程是并行执行的，每个子线程执行完后countDown()一次，state会CAS(Compare and Swap)减1。等到所有子线程都执行完后(即state=0)，会unpark()主调用线程，然后主调用线程就会从await()函数返回，继续后余动作。
